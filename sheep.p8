...
function tickplayer(p)
  ...
  if p.sleep then
    p.sleep -= 1
    if (p.sleep == 0) p.sleep = nil return
  end
...
function ticksheep(e)
  ...
  if e.t == 0 then
    ...
    if w then
      ...
      local adx = abs(dx)
      local ady = abs(dy)
      if (adx > 5) e.mx = -sgn(dx) if (ady > 5) e.my = -sgn(dy) e.t = 30
      return
    end
    ...
    if e.friend then
      ...
      if (adx > 10) e.mx = sgn(dx) if (ady > 10) e.my = sgn(dy) return
    end
    ...
    if still then
      ...
      if e.fellow then
        ...
        if follow then
          ...
          local adx = abs(dx)
          local ady = abs(dy)
          if (adx > 2) e.mx = sgn(dx) if (ady > 2) e.my = sgn(dy) e.t = 30
          return
        end
      end
